...
In order to access my drive, I'll be using the authorization function from googlesheets4, `gs4_auth()`, which allows you to either interactively select a pre-authorized account in R, or takes you to the browser to generate obtain a new token for your account.
...
Now that we've separated our data out, let's use some of the `readr::parse_*()` functions (handy even when you're not reading the data in) to get the data types right. Using `readr::parse_number()` is especially nice when working with numeric data that has any character in front of or after the numbers themselves.
